...
namespace WishListTests
{
    public class AddMVCMiddlewareTests
    {
        ...
        public void AddMVCCallAdded()
        {
            ...
            Assert.True(file.Contains("services.AddMvc();"),
                "`Startup.cs`'s `ConfigureServices` method did not contain a call to `AddMvc`.");
        }
        ...
        public void UseEndpointsAdded()
        {
            ...
            var pattern =
                @"app.UseRouting\s*?[(]\s*?[)]\s*?;\s*?app.UseEndpoints\s*?[(]\s*?endpoints\s*?=>\s*?{\s*?endpoints.MapDefaultControllerRoute\s*?[(]\s*?[)]\s*?;\s*?}\s*?[)]\s*?;";
            var rgx = new Regex(pattern);
            Assert.True(rgx.IsMatch(file),
                "`Startup.cs`'s `Configure` method did not contain a call to `UseEnpoints` on `app` after `UseRouting` with an argument of `endpoints => { endpoints.MapDefaultControllerRoute(); }`");
        }
    }
}

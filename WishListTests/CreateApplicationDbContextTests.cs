    ...
    public class CreateApplicationDbContextTests
    {
        ...
        public void CreateApplicationDbContextTest()
        {
            ...
            Assert.True(applicationDbContext.BaseType == typeof(DbContext),
                "`ApplicationDbContext` was found, but did not inherit the `DbContext` class. (this will require a using directive for the `Microsoft.EntityFrameWorkCore` namespace)");
        }
        ...
        public void AddConstructorToApplicationDbContextTest()
        {
            ...
            var constructor = applicationDbContext.GetConstructor(new[] {typeof(DbContextOptions)});
            Assert.True(constructor != null,
                "`ApplicationDbContext` does not appear to contain a constructor accepting a parameter of type `DbContextOptions<ApplicationDbContext>`");
        }
    }
}
